...
var DataArr = La.map(function (element) {
    ...
    for (var i = 0; i < element.length; i++) {
        ...
        var maxByPos = SortedStats['l' + i][0].Count;
        if (maxByPos < 1) maxByPos = 500;
        SortedStats['l' + i].forEach(function (item) {
            if (item.letter == character) ScoreByPos += Math.round( (item.Count * 100) / maxByPos );
        });
    }
    wordObj.Score = score;
    wordObj.PosScore = ScoreByPos;
    return wordObj;
});
...
var DataArrLewdle = lewdle.map(function (element) {
    ...
    for (var i = 0; i < element.length; i++) {
        ...
        var maxByPos = SortedStatsLewdle['l' + i][0].Count;
        if (maxByPos < 1) maxByPos = 500;
        SortedStatsLewdle['l' + i].forEach(function (item) {
            if (item.letter == character) ScoreByPos += Math.round( (item.Count * 100) / maxByPos );
        });
    }
    wordObj.Score = score;
